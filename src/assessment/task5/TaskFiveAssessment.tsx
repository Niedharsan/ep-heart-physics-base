...
const caseHeadings: Readonly<Record<TaskFiveCaseId, string>> = Object.freeze({
  ...
  'para-hisian': 'Para-Hisian pacing EGM',
});
...
const studentTraceDescriptions: Readonly<Record<TaskFiveCaseId, string>> = Object.freeze({
  'vt-rvot': 'Synthetic six-lead ECG excerpt for ventricular-tachycardia morphology interpretation.',
  'vt-fascicular': 'Synthetic six-lead ECG excerpt for ventricular-tachycardia morphology interpretation.',
  'para-hisian': 'Synthetic intracardiac recording comparing higher-output and lower-output pacing states.',
});
...
export function TaskFiveAssessment({ assessmentView }: { readonly assessmentView: AssessmentView }) {
  ...
  const submit = (): void => {
    ...
    setCopyStatus(`Task 5 attempt saved locally: ${nextResult.score}/15.`);
  };
  ...
  const resetAnswers = (): void => {
    ...
    setFeedbackNotes('');
    setCopyStatus('Task 5 answers reset. Saved attempt history was kept.');
  };

  const copyFeedbackPackage = async (): Promise<void> => {
    if (!result || !latestAttemptId) {
      setCopyStatus('Mark Task 5 before copying a feedback package.');
      return;
    }
    ...
    try {
      ...
      setCopyStatus('Task 5 feedback package copied. Paste it into email or your project message.');
    } catch {
    ...
    <main className="assessment-shell task-five-shell">
      ...
      <header className="assessment-header">
        <div>
          ...
          <p>
            Interpret two deterministic synthetic VT ECG examples and a paired para-Hisian pacing
            EGM demonstrating capture-state, timing and retrograde-sequence comparisons.
          </p>
        </div>
      ...
      <nav className="assessment-task-nav" aria-label="Assessment sections">
        ...
        <a className="active" href={taskHref('5', instructor)}>Task 5 · VT & para-Hisian pacing</a>
      </nav>

      <div className="prototype-warning">
        Synthetic educational traces only. ECG morphology and para-Hisian responses have recognised mimics and limitations and must not be used for patient diagnosis.
      </div>
      {instructor && (
      ...
      <section className="task-five-grid" aria-label="Task 5 ECG and EGM cases">
        {taskFiveCases.map((taskCase, index) => {
          ...
          return (
            <article className={`assessment-panel task-five-card ${taskCase.id === 'para-hisian' ? 'task-five-wide' : ''}`} key={taskCase.id}>
              <div className="assessment-panel-heading">
                <div>
                  <span>{taskCase.id === 'para-hisian' ? 'EGM' : 'ECG'} CASE {index + 1} · {taskCase.maximumScore} MARKS</span>
              ...
              <label>
                ...
                <textarea
                  ...
                  placeholder={taskCase.id === 'para-hisian'
                    ? 'Describe the capture transition, comparisons, response patterns and limitation.'
                    : 'State the likely VT subtype or origin and the supporting ECG morphology.'}
                />
              </label>
              {instructor && (
      ...
      <section className="assessment-grid lower-grid task-five-section">
        <article className="assessment-panel">
          <div className="assessment-panel-heading">
            ...
            <button onClick={() => { clearTaskFiveAttempts(); setAttempts([]); }}>Clear history</button>
          </div>
          {attempts.length === 0 ? (
