...
import {
  AlievPanfilovModel,
  alievPanfilovPresets,
  defaultAlievPanfilovParameters,
} from '../engine/models/AlievPanfilov';

const generalizedPreset = alievPanfilovPresets.goktepeKuhl2009Figure4Generalized;

const derivativeFixtures = [
  { state: [0, 0], expected: [0, 0] },
  { state: [0.01, 0.002], expected: [-0.0031880000000000003, 0.0002934967741935483] },
  { state: [0.4, 0.1], expected: [0.632, 0.07031428571428573] },
  { state: [1.1, 0.35], expected: [-1.309000000000001, 0.0046799999999999715] },
  { state: [-0.19, 1.9], expected: [0.795112, -13.607728145454546] },
] as const;

describe('AlievPanfilovModel', () => {
  it('matches independently derived reaction derivatives', () => {
    const model = new AlievPanfilovModel(generalizedPreset);
  ...
  it('counts the defensive denominator guard without disguising it as the published equation', () => {
    const model = new AlievPanfilovModel(generalizedPreset);
    model.derivatives(-generalizedPreset.mu2, 0.1);
  ...
  it('validates every project-supported parameter domain with named errors', () => {
    ...
    for (const [name, value] of invalid) {
      expect(() => new AlievPanfilovModel({ ...generalizedPreset, [name]: value }))
    ...
    for (const name of names) {
      ...
      const missing = { ...generalizedPreset } as Record<string, number>;
      delete missing[name];
  ...
  it('snapshots validated parameters so caller mutation cannot bypass validation', () => {
    const supplied = { ...generalizedPreset };
  ...
  it('exposes immutable source-named presets and a compatibility alias to the generalized preset', () => {
    expect(generalizedPreset).toEqual({ a: 0.05, b: 0.15, k: 8, epsilon: 0.002, mu1: 0.2, mu2: 0.3 });
    expect(alievPanfilovPresets.alievPanfilov1996Classic)
      .toEqual({ a: 0.15, b: 0.15, k: 8, epsilon: 0.002, mu1: 0.2, mu2: 0.3 });
    expect(defaultAlievPanfilovParameters).toBe(generalizedPreset);
    expect(Object.isFrozen(alievPanfilovPresets)).toBe(true);
    expect(Object.isFrozen(generalizedPreset)).toBe(true);
  });

  it('reduces the generalized recovery equation to the classic single-a form when b equals a', () => {
    const preset = alievPanfilovPresets.alievPanfilov1996Classic;
    const model = new AlievPanfilovModel(preset);
    const u = 0.4;
    const v = 0.1;
    const expectedDv = (preset.epsilon + preset.mu1 * v / (u + preset.mu2))
      * (-v - preset.k * u * (u - preset.a - 1));
    expect(model.derivatives(u, v)[1]).toBeCloseTo(expectedDv, 14);
  });

  it('documents that the former recovery cap intersects the sourced model nullcline', () => {
    const { b, k } = generalizedPreset;
    const nullclinePeak = k * (1 + b) ** 2 / 4;
    expect(nullclinePeak).toBeCloseTo(2.645, 12);
    expect(nullclinePeak).toBeGreaterThan(2);
  });
});
